...
function draw() {
  ...
  var img = new Image()
  img.src =
    'http://img0.imgtn.bdimg.com/it/u=332248981,2934284699&fm=26&gp=0.jpg'
  //图片加载完后，将其显示在canvas中
  img.onload = function () {
...
function Snake(x, y) {
  ...
  this.move = function () {
    ...
    this.time = setInterval(function () {
      ...
      if (
        _this.body[0][0] < 0 ||
        _this.body[0][0] > width / data.size - 1 ||
        _this.body[0][1] < 0 ||
        _this.body[0][1] > height / data.size - 1
      ) {
        alert('你撞墙了')
        _this.stop()
      }
